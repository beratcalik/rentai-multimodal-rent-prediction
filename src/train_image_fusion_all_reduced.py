...
LOGGER = logging.getLogger("image_fusion_all_reduced_training")

ROOT_DIR = Path(__file__).resolve().parent.parent
MULTIMODAL_DATASET_PATH = ROOT_DIR / "dataset" / "train_ready_multimodal.parquet"
IMAGE_EMBEDDINGS_PATH = ROOT_DIR / "dataset" / "image_embeddings_all.parquet"
MODEL_OUTPUT_PATH = ROOT_DIR / "models" / "image_fusion_all_reduced_model.joblib"
REPORT_OUTPUT_PATH = ROOT_DIR / "reports" / "image_fusion_all_reduced_results.md"

SIX_IMAGE_REFERENCE_METRICS = {
    "mae": 4555.73,
    "rmse": 6842.20,
    "r2": 0.8133,
    "mape": 13.53,
...
def load_and_join_datasets(
    ...
    LOGGER.info("All-image embeddings okunuyor: %s", embeddings_path)
    ...
    required_embedding_columns = {"listing_id", IMAGE_EMBEDDING_COLUMN, USED_IMAGE_COUNT_COLUMN}
    ...
    embeddings_expanded_df = pd.concat(
        [
            embeddings_df.loc[:, ["listing_id", USED_IMAGE_COUNT_COLUMN]].reset_index(drop=True),
            embedding_expanded_df,
        ],
        axis=1,
    )
...
def train_validation_trials(
    ...
    if not results:
        raise RuntimeError("Hicbir all-image reduced fusion modeli basariyla egitilemedi.")
...
def build_prediction_frame(
    ...
    columns = ["listing_id", "district", "neighborhood", "rooms", "m2_gross", USED_IMAGE_COUNT_COLUMN]
    frame = test_df.loc[:, columns].copy()
    ...
    frame[USED_IMAGE_COUNT_COLUMN] = pd.to_numeric(frame[USED_IMAGE_COUNT_COLUMN], errors="coerce")
    return frame
...
def summarize_used_image_count(prediction_frame: pd.DataFrame) -> pd.DataFrame:
    grouped = (
        prediction_frame.dropna(subset=[USED_IMAGE_COUNT_COLUMN])
        .groupby(USED_IMAGE_COUNT_COLUMN)
        .agg(
            sample_count=("listing_id", "size"),
            baseline_mae=("baseline_abs_error", "mean"),
            fusion_mae=("fusion_abs_error", "mean"),
            baseline_mape=("baseline_ape_pct", "mean"),
            fusion_mape=("fusion_ape_pct", "mean"),
            avg_actual_price=("actual_price_try", "mean"),
        )
        .reset_index()
        .rename(columns={USED_IMAGE_COUNT_COLUMN: "used_image_count"})
    )
    grouped["mae_improvement"] = grouped["baseline_mae"] - grouped["fusion_mae"]
    grouped["mape_improvement"] = grouped["baseline_mape"] - grouped["fusion_mape"]
    grouped["used_image_count"] = grouped["used_image_count"].astype(int)
    return grouped.sort_values("used_image_count").reset_index(drop=True)
...
def build_matched_baseline_comparison_table(
    ...
    rows = [
        ...
        {
            "row": "All-image reduced fusion model",
...
def build_six_image_comparison_table(
    six_image_reference_metrics: dict[str, float],
    fusion_metrics: dict[str, float],
) -> pd.DataFrame:
    rows = [
        {
            "row": "6-image reduced fusion reference",
            "mae": six_image_reference_metrics["mae"],
            "rmse": six_image_reference_metrics["rmse"],
            "r2": six_image_reference_metrics["r2"],
            "mape": six_image_reference_metrics["mape"],
        },
        {
            "row": "All-image reduced fusion model",
            "mae": fusion_metrics["mae"],
        ...
        {
            "row": "Improvement vs 6-image reduced fusion",
            "mae": six_image_reference_metrics["mae"] - fusion_metrics["mae"],
            "rmse": six_image_reference_metrics["rmse"] - fusion_metrics["rmse"],
            "r2": fusion_metrics["r2"] - six_image_reference_metrics["r2"],
            "mape": six_image_reference_metrics["mape"] - fusion_metrics["mape"],
        },
    ]
    return pd.DataFrame(rows)
...
def render_optional_model_status(optional_model_tracking: dict[str, dict[str, Any]]) -> str:
    lines: list[str] = []
    for model_name in ["XGBRegressor", "LightGBMRegressor"]:
...
def build_result_comment(
    ...
    if fusion_metrics["mae"] < baseline_subset_metrics["mae"]:
        lines.append(
            f"- All-image fusion, matched tabular baseline'i MAE tarafinda {baseline_subset_metrics['mae'] - fusion_metrics['mae']:.2f} puan geciyor."
        )
    else:
        lines.append(
            f"- All-image fusion, matched tabular baseline'in gerisinde kaliyor; MAE farki {fusion_metrics['mae'] - baseline_subset_metrics['mae']:.2f}."
        )

    if fusion_metrics["mae"] < six_image_reference_metrics["mae"]:
        lines.append(
            f"- All-image embeddingler, 6-image reduced fusion referansina gore MAE'yi {six_image_reference_metrics['mae'] - fusion_metrics['mae']:.2f} iyilestiriyor."
        )
    else:
        lines.append(
            f"- All-image embeddingler, 6-image reduced fusion referansina gore MAE'yi {fusion_metrics['mae'] - six_image_reference_metrics['mae']:.2f} kotulestiriyor."
        )

    if image_ablation_metrics["mae"] > fusion_metrics["mae"]:
        lines.append(
            f"- Image branch ablasyonu sonrasinda MAE {image_ablation_metrics['mae'] - fusion_metrics['mae']:.2f} kotulesti; reduced image block net pozitif katkili."
        )
    else:
        lines.append(
            f"- Ablasyon sonrasinda MAE {fusion_metrics['mae'] - image_ablation_metrics['mae']:.2f} iyilesti; image branch sinyali henuz tam verimli kullanilmiyor olabilir."
        )
...
def build_report(
    ...
    fusion_metrics: dict[str, float],
    baseline_subset_metrics: dict[str, float],
    matched_baseline_comparison_df: pd.DataFrame,
    six_image_comparison_df: pd.DataFrame,
    ...
    optional_model_tracking: dict[str, dict[str, Any]],
    used_image_count_stats: dict[str, float],
) -> str:
    optional_model_section = render_optional_model_status(optional_model_tracking)
    result_comment_lines = build_result_comment(
        baseline_subset_metrics=baseline_subset_metrics,
        six_image_reference_metrics=SIX_IMAGE_REFERENCE_METRICS,
    ...
    report_lines = [
        ...
        f"- All-image embedding source: `{IMAGE_EMBEDDINGS_PATH}`",
        ...
        f"- used_image_count ortalamasi: **{used_image_count_stats['mean']:.2f}**",
        f"- used_image_count medyani: **{used_image_count_stats['median']:.2f}**",
        f"- used_image_count araligi: **{int(used_image_count_stats['min'])} - {int(used_image_count_stats['max'])}**",
        "",
        ...
        "",
        dataframe_to_markdown_table(matched_baseline_comparison_df, digits=4),
        "",
        "## 6-Image vs All-Image Fusion",
        "",
        dataframe_to_markdown_table(six_image_comparison_df, digits=4),
        "",
        ...
        f"- All-image reduced fusion MAE: **{fusion_metrics['mae']:.2f}**",
...
def main() -> int:
    ...
    used_image_count_df = summarize_used_image_count(prediction_frame)
    ...
    matched_baseline_comparison_df = build_matched_baseline_comparison_table(
        baseline_subset_metrics=baseline_subset_metrics,
        fusion_metrics=fusion_metrics,
    )
    six_image_comparison_df = build_six_image_comparison_table(
        six_image_reference_metrics=SIX_IMAGE_REFERENCE_METRICS,
        fusion_metrics=fusion_metrics,
    )

    used_image_count_series = pd.to_numeric(joined_df[USED_IMAGE_COUNT_COLUMN], errors="coerce")
    used_image_count_stats = {
        "mean": float(used_image_count_series.mean()),
        "median": float(used_image_count_series.median()),
        "min": float(used_image_count_series.min()),
        "max": float(used_image_count_series.max()),
    }
    ...
    report_body = build_report(
        ...
        fusion_metrics=fusion_metrics,
        baseline_subset_metrics=baseline_subset_metrics,
        matched_baseline_comparison_df=matched_baseline_comparison_df,
        six_image_comparison_df=six_image_comparison_df,
        ...
        optional_model_tracking=optional_model_tracking,
        used_image_count_stats=used_image_count_stats,
    )
