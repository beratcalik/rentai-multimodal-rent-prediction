...
LOGGER = logging.getLogger("image_embedding_extraction_all")

ROOT_DIR = Path(__file__).resolve().parent.parent
DEFAULT_INPUT_PATH = ROOT_DIR / "dataset" / "train_ready_multimodal.parquet"
DEFAULT_OUTPUT_PATH = ROOT_DIR / "dataset" / "image_embeddings_all.parquet"
DEFAULT_REPORT_PATH = ROOT_DIR / "reports" / "image_embedding_all_report.md"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract EfficientNet-B0 image embeddings per listing using all readable images."
    )
...
def extract_embeddings(
    ...
    for row in progress_bar:
        ...
        progress_bar.set_postfix(
            ...
            used_images=sum(used_image_counts.values()),
        )
