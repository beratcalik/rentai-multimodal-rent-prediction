...
LOGGER = logging.getLogger("siglip_image_embedding_extraction")

ROOT_DIR = Path(__file__).resolve().parent.parent
DEFAULT_INPUT_PATH = ROOT_DIR / "dataset" / "train_ready_multimodal.parquet"
DEFAULT_OUTPUT_PATH = ROOT_DIR / "dataset" / "siglip_image_embeddings.parquet"
DEFAULT_REPORT_PATH = ROOT_DIR / "reports" / "siglip_image_embedding_report.md"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract SigLIP image embeddings per listing using up to 16 readable images."
    )
...
def flush_batch(
    ...
    pixel_values = batch_inputs["pixel_values"].to(device, non_blocking=device.type == "cuda")
    ...
    output_array = outputs.pooler_output.detach().float().cpu().numpy()
    ...
    processed_image_count = len(batch_images)
...
def extract_embeddings(
    ...
    for row in progress_bar:
        ...
        for raw_path in raw_paths:
            ...
            try:
                image = load_pil_image(resolved_path)
            except Exception:
                stats.broken_image_count += 1
                continue
    ...
    for listing_id in dataframe[LISTING_ID_COLUMN].astype(str).tolist():
        ...
        rows.append(
            {
                LISTING_ID_COLUMN: listing_id,
                "siglip_mean_embedding": mean_embedding.tolist(),
                "siglip_max_embedding": max_embedding.tolist(),
                "siglip_meanmax_embedding": meanmax_embedding.tolist(),
    ...
    if not embedding_df.empty:
        stats.embedding_dimension = int(len(embedding_df.iloc[0]["siglip_mean_embedding"]))
        stats.meanmax_dimension = int(len(embedding_df.iloc[0]["siglip_meanmax_embedding"]))
...
def build_report(
    ...
    lines = [
        ...
        f"- Encoder: `transformers` / `{SIGLIP_MODEL_ID}`",
...
def build_model(device: torch.device) -> tuple[SiglipVisionModel, Any]:
    LOGGER.info("SigLIP model yukleniyor: %s", SIGLIP_MODEL_ID)
    processor = AutoImageProcessor.from_pretrained(SIGLIP_MODEL_ID)
    model = SiglipVisionModel.from_pretrained(SIGLIP_MODEL_ID)
    model.to(device)
    model.eval()
