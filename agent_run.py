...
async def run(args):
    ...
    search_url = f"{args.search_url}?page={args.start_page}" if args.start_page > 1 else args.search_url
    ...
    async with async_playwright() as p:
        ...
        try:
            await warmup_session(page, search_url)
        except Exception as e:
            log({"ts": now_iso(), "event": "warmup_failed", "error": str(e)})

        if not args.no_discover:
            try:
                discovered = await harvest_listing_urls(page, search_url, max_pages=args.max_pages)
            ...
            print("[queue] discovery skipped, using existing queue")
...
def build_parser():
    ...
    ap.add_argument("--start-page", type=int, default=1, help="Start discovery from this results page")
    ...
    ap.add_argument("--no-discover", action="store_true", help="Skip listing discovery")
    return ap
